...
class CreditPersonSpider(object):
    ...
    def write2csv(self, items):
        path_dir = self.savePath.split("/")[0]
    ...
    def test(self):
        ...
        print(self.hasNext)
        # 获取信息
        items = self.get_credict_person_info(json_str.get("data")[0]['result'])
        self.write2csv(items)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 3:
        print("input error!")
        print("you should input like this: python spider.py --pageNum=1 --savePath=./data/data1.csv")
    elif sys.argv[1].split("=")[0] != "--pageNum" or sys.argv[2].split("=")[0] != "--savePath":
        print("you should input the parameter1 --pageNum the parameter2 --savePath")
    else:
        print(sys.argv[0], sys.argv[1], sys.argv[2])
        key, pageNum = sys.argv[1].split("=")
        key2,savePath = sys.argv[2].split("=")
